...
#define UpdateURl	"https://drive.google.com/file/d/1I8fz1dhmWfWa5gqo3tw5IbGv_EE_S_oK/view?usp=sharing"
...
public Action OnCustomStatusHUDUpdate(int client, StringMap entries) 
{
	if (IsValidClient(client))
	{
		...
		if(WepAttribCheck(clientweapon, "void mangler"))
		{
			Format(totaldamage, sizeof(totaldamage), "Current DMG Mult: %.2f", Totaldamage[client]);
			entries.SetString("uu_weapons_mangler_dmgmult", totaldamage);
		}
		if(WepAttribCheck(clientweapon, "scorch shot extreme"))
		{
			Format(emberstacks, sizeof(emberstacks), "Ember: %.0f", Ember_stacks[client]);
			entries.SetString("uu_weapons_scorchshot_ember", emberstacks);
		}
			...
			if (!WepAttribCheck(clientweapon, "cw3 weapon"))
			{
				...
				switch(ItemDefinition)
				{
					...
					case 441:
					{
						...
						TF2Attrib_SetByName(clientweapon3, "damage penalty", 0.15);
						TF2Attrib_SetByName(clientweapon3, "flat damage reduction", 30.0);
		...
		if (IsValidEntity(clientweapon3))
		{
			...
			switch(ItemDefinition)
			{
				case 593:
				{
					...
					TF2Attrib_SetByName(clientweapon3, "flat damage reduction", 300.0);
				...
				case 348:
				{
					...
					TF2Attrib_SetByName(clientweapon3, "damage penalty vs player", 0.20);
					...
					//TF2Attrib_SetByName(clientweapon3, "flat damage reduction", 150.0);
