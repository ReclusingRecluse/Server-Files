									...
									if (Filter2 != INVALID_HANDLE)
									{
										if (!TR_DidHit(Filter2))
										{
											if (IsValidTarget(i, client))
											{
												...
												Hits[i] += 2.0;
												new Float:Damage = 35.0*(Hits[i]*1.30);
												SDKHooks_TakeDamage(i, clientweapon, client, Damage, DMG_BURN, -1, NULL_VECTOR, NULL_VECTOR, false);
	...
	if (IsValidClient(client))
	{
		...
		if (IsValidEntity(clientweapon))
		{
			if (WepAttribCheck(clientweapon, "solar flare"))
			{
				...
				{
					Radius[client] = 270.0;
					Radius[client] *= GetWepAttribValue(clientweapon, "Blast radius increased custom");
				}
				else
				{
					return;
