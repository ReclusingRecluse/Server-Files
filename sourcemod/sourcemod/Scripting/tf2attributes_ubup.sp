			...
			if (IsValidEntity(C_Weapon))
			{
				...
				if (TF2_GetPlayerClass(client) == TFClass_Sniper)
				{
					...
					if (slot == 0)
					{
						...
						else if (WepAttribCheck(C_Weapon, "le monarque"))
						{
							currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_le_monarque");
						}
						else
						{
							currentitem_catidx[client][slot] = GetUpgrade_CatList(classname);
				...
				if (TF2_GetPlayerClass(client) == TFClass_Soldier)
				{
					...
					if (slot == 0)
					{
						...
						else if (WepAttribCheck(C_Weapon, "arcstrike"))
						{
							currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_rocketlauncher_arcstrike");
						}
						else
						{
							currentitem_catidx[client][slot] = GetUpgrade_CatList(classname);
...
//		TF2_RespawnPlayer(client);
	...
	if(!IsMvM())
	{
		...
		{
			PrintToChatAll("Money Cap has been reached, kills will no longer give money.");
			//CreateTimer(50.0, BotDifficulty, _, TIMER_REPEAT);
...
public Action:BotDifficulty(Handle:Timer)
{
	if(MoneyCap_active)
	{
		...
		new Float:NewDiff = CurrentDiff+0.05;
		...
		if (s >= 0 && s < 5 && param2 < MAX_ATTRIBUTES_ITEM)
		{
			if (param2 >= 0)
			{
				...
				if (u != 20000)
				{
					...
					else if (upgrades_costs[u] > 1.0 && upgrades_removable[u] == 1)
					{
						new nb_time_upgraded;
						...
						CurrencyOwned[client] += up_cost;
						client_spent_money[client][s] -= up_cost
						//PrintToChat(client,"Removed attribute from slot %i", s);
					}
					else
					{
