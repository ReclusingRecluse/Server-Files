...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int maxlen) {
	RegPluginLibrary("custom_status_hud_Elements");
	return APLRes_Success;
}

public void OnPluginStart() {
	g_SyncDisplay = CreateHudSynchronizer();
	g_HUDUpdateForward = CreateGlobalForward("OnCustomStatusHUDUpdate2", ET_Event, Param_Cell,
			Param_Cell);
	
	g_PrefHudXPos = RegClientCookie("statushudelements_xpos",
			"Preferred X position for the Custom Status HUD", CookieAccess_Public);
	g_PrefHudYPos = RegClientCookie("statushudelements_ypos",
			"Preferred Y position for the Custom Status HUD", CookieAccess_Public);
	g_PrefHudColor = RegClientCookie("statushudelements_color",
	...
	*/
	...
	CreateTimer(0.1, HudRefresh, _, TIMER_REPEAT);
}

public Action:HudRefresh(Handle:Timer)
{
...
void GetClientHudTextParams(int client, float &x, float &y, int &color) {
	...
	x = buffer[0]? StringToFloat(buffer) : -0.10;
	
	GetClientCookie(client, g_PrefHudYPos, buffer, sizeof(buffer));
	y = buffer[0]? StringToFloat(buffer) : 0.40;
