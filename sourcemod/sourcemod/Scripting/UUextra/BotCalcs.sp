			...
			if (IsValidClient(client) && Scaleactive >= 1.0)
			{
				if (IsFakeClient(client))
				{
					...
					if (IsFakeClient(client))
					{
						...
						TF2Attrib_SetByName(client, "tmp dmgbuff on hit", 30.0);
						...
						if (!IsMvM())
						{
							...
							TF2Attrib_SetByName(client, "obsolete ammo penalty", ((resistbuy[client]*3.50)+2000.0));
						}
						if (IsMvM())
						{
							TF2Attrib_SetByName(client, "disguise on backstab", (regenbuy[client]*0.05));
							TF2Attrib_SetByName(client, "obsolete ammo penalty", ((resistbuy[client]*3.50)+1800.0));
						}
						...
						TF2Attrib_SetByName(client, "referenced item id low", 0.09);
						...
						{
							TF2Attrib_SetByName(BotWeapon, "cannot giftwrap", damagebuy[client]*1.20);
							TF2Attrib_SetByName(BotWeapon, "tool needs giftwrap", damagebuy2[client]*1.20);
							TF2Attrib_SetByName(BotWeapon, "damage bonus", 2.00);
