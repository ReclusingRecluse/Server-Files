...
public Action OnCustomStatusHUDUpdate2(int client, StringMap entries) 
{
	if (IsValidClient(client))
	{
		...
		if (Supernova_active[client])
		{
			Format(Supernovabonus, sizeof(Supernovabonus), "1.75x Blast Damage Bonus");
			entries.SetString("elements_effects_supernova_bonus", Supernovabonus);
		}
	}
	return Plugin_Changed;
}
...
public Event_Playerhurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsValidClient(killer))
	{
		...
		if (Reflect_active[victim] == true && Current_HealthPool[victim] < Reflect_HealthPool[victim] && current_charge[victim] > Reflect_cost[victim])
		{
			...
			new Float:HealthAdd = damage*0.40;
			Current_HealthPool[victim] += HealthAdd;
		}
		if (Reflect_active[victim] == true && Reflect_dmgreflectactive[victim] == true && current_charge[victim] > Reflect_cost[victim]*2.0)
		{
			if (IsValidClient(victim) && IsValidClient(killer))
			{
				...
				if (IsValidEntity(ClientWeapon))
				{
					...
					current_charge[victim] -= Reflect_cost[victim];
					SDKHooks_TakeDamage(killer, ClientWeapon, victim, Reflect_Dmg, DMG_SLASH, ClientWeapon, NULL_VECTOR, NULL_VECTOR, true);
				}
			}
		}
	}
