...
#include "Elements/PlayerDeath.sp"
//#include "Elements/Natives.sp"

//#include "Elements/Shields.sp"
//#include "Elements/Hud.sp"

new Handle:h_ElementForward;
public OnMapStart()
{
...
//Index for Elemental weapon using weapon slot
int g_ElementalWeapon[MAXPLAYERS+1][5];

//Properly Assign Weapons Elements via integer value
stock AssignWeaponElement(int client, int slot)
{
	...
	if (WepAttribCheck(slot, "scorch"))
	{
		selection = 1;
	}
	if (WepAttribCheck(slot, "throwable particle trail only"))
	{
		selection = 2;
	}
	if (WepAttribCheck(slot, "strange restriction user value 3"))
	{
		selection = 3;
	}
	if (WepAttribCheck(slot, "stasis element"))
	{
		selection = 4;
	}
	if (WepAttribCheck(slot, "strand element"))
	{
		selection = 5;
	}
	else
	...
	g_ElementalWeapon[client][slot] = selection;
	PrintToChat(client, "Assigned Slot %d Element %d", slot, g_ElementalWeapon[client][slot]);
...
public OnClientPutInServer(client)
{
	if (IsValidClient(client))
	{
		...
		SDKHook(client, SDKHook_OnTakeDamage, OnTakeDamageStasis);
		...
		IsVolatile[client] = false;
		Volatile_CurrentDMG[client] = 0.0;
		Volatile_DmgThresh[client] = 0.0;
	}
}
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int maxlen) {
	...
	/*
	CreateNative("GetElementalDamageType", Native_ElmtlDmgType);
	CreateNative("DE_IsScorched", Native_IsScorched);
	CreateNative("DE_GetScorchAmount", Native_GetScorchAmount);
	CreateNative("DE_IsIgnitionDMG", Native_IsIgnitionDMG);
	
	CreateNative("DE_IsSlowed", Native_IsSlowed);
	CreateNative("DE_GetSlowAmount", Native_GetSlowAmount);
	CreateNative("DE_IsShatterDMG", Native_IsShatterDMG);
	
	CreateNative("DE_IsPoisoned", Native_IsPoisoned);
	CreateNative("DE_UnravelingRoundsActive", Native_IsUnravelingRoundsActive);
	*/
...
public OnPluginStart()
{
	...
	h_ElementForward = CreateGlobalForward("Elements_OnConditionAdded", ET_Event, Param_Cell, Param_Cell, Param_String, Param_Cell);
	//Register Attributes
	...
	attrib.SetName("weaken on damage chain");
	attrib.SetClass("element_arc_weaken_on_chain_dmg");
	attrib.SetDescriptionFormat("value_is_additive");
	attrib.Register();
	
	attrib.SetName("is weakened");
	attrib.SetClass("element_arc_weaken_detection");
	attrib.SetDescriptionFormat("value_is_additive");
	attrib.Register();
	
	attrib.SetName("polyute");
	attrib.SetClass("element_arc_polyute");
	attrib.SetDescriptionFormat("value_is_additive");
	attrib.Register();
	
	attrib.SetName("chain damage additive bonus");
	attrib.SetClass("element_arc_chain_additive_dmg_bonus");
	attrib.SetDescriptionFormat("value_is_additive");
	attrib.Register();
	
	attrib.SetName("unraveling rounds");
	attrib.SetClass("element_strand_unraveling_rounds");
	attrib.SetDescriptionFormat("value_is_additive");
	attrib.Register();
	...
	for ( new i = 1; i <= MaxClients; i++ )
	{
		if (IsValidClient(i))
		{
			...
			CanBeVoided[i] = true;
			IsVolatile[i] = false;
			Volatile_CurrentDMG[i] = 0.0;
			Volatile_DmgThresh[i] = 0.0;
...
public Action PlayerRespawn(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (IsValidClient(client))
	{
		...
		Frozen_Delay[client] = 0.0;
		CreateTimer(0.4, SetShatterThreshhold, client);
...
public Action:SetShatterThreshhold(Handle:Timer, any:client)
{
	if (IsValidClient(client))
	{
		Dmg_TilShatter[client] = (TF2_GetMaxHealth(client)*0.70);
		
		for (int i = 0; i <= 2; i++)
		{
			new slot = GetPlayerWeaponSlot(client, i);
			if (i == slot && IsValidEntity(slot))
			{
				AssignWeaponElement(client, slot);
			}
		}
	}
}
...
public Action:OnCustomStatusHUDUpdate2(int client, StringMap entries) 
{
	if (IsValidClient(client))
	{
		//Create chars for  HUD text
		char stats[32];
		char str_Scorch[32];
		char str_Slowed[32];
		char str_Frozen[32];
		char str_Poisoned[32];
		char str_Volatile[32];
		char str_Weakened[32];
		char str_Unrav[32];
		...
		if (Weakened[client])
		{
			Format(str_Weakened, sizeof(str_Weakened), "Weakened");
			entries.SetString("elements_effects_arc_weakened", str_Weakened);
		}
		if (Unraveling_Rounds_Active[client])
		{
			Format(str_Unrav, sizeof(str_Unrav), "Unraveling Rounds x%.0f", Unraveling_Rounds_Stacks[client]);
			entries.SetString("elements_effects_strand_unrav", str_Unrav);
		}
...
{
	if (!IsValidClient(c_victim) || !IsPlayerAlive(c_victim) || !IsValidEntity(entity))
		return false;
