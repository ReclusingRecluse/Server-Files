...
public Action:Event_Death(Handle:Event, const String:strName[], bool DontBroadcast)
{
	...
	if (victim != killer)
	{
		...
		new LandTankGun = GetEntPropEnt(killer, Prop_Send, "m_hActiveWeapon");
		if (IsValidEntity(LandTankGun))
		{
			new Address: LandTankActive = TF2Attrib_GetByName(LandTankGun, "strange score selector");
			if (LandTankActive!=Address_Null)
			{
				TF2_AddCondition(killer, TFCond_RuneResist, 3.0);
				CreateTimer(3.0, Timer_MoveSpeedRemove, LandTankGun);
				TF2Attrib_SetByName(LandTankGun, "move speed penalty", 0.70);
			}
		}
	}			
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
	if (IsValidClient(attacker))
	{
		...
		if (victim != attacker && IsValidEntity(melee))
		{
...
public Action:Timer_MoveSpeedRemove(Handle:Timer, any:LandTankGun)
{
	if (IsValidEntity(LandTankGun))
	{
		TF2Attrib_RemoveByName(LandTankGun, "move speed penalty");
	}
	return Plugin_Changed;
}
