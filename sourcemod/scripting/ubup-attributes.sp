...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
	...
	if(damage < 0.0)// Make sure you can't deal negative damage....
	{
		...
		/*
		PrintToChat(victim, "weapon %i", weapon);
		PrintToChat(victim, "attacker %i", attacker);
		PrintToChat(attacker, "victim %i", victim);
		PrintToChat(victim, "inflictor %i", inflictor);
		PrintToChat(victim, "damagetype %i", damagetype);
		PrintToChat(victim, "damagecustom %i", damagecustom);
		PrintToChatAll(" ");
    ...
    { 
		...
		if(IsValidClient(client))
		{
			...
			if(IsValidEntity(ClientWeapon))
			{
				...
				new Address:projspeed2 = TF2Attrib_GetByName(ClientWeapon, "Projectile speed increased HIDDEN");
				if(projspeed != Address_Null){
					new Float:vAngles[3];
				...
				if(projspeed2 != Address_Null){
					...
					GetEntPropVector(entity, Prop_Data, "m_vecAbsVelocity", vel);
					GetAngleVectors(vAngles, vBuffer, NULL_VECTOR, NULL_VECTOR);
					new Float:projspd = TF2Attrib_GetValue(projspeed2);
					vVelocity[0] = vBuffer[0]*projspd*GetVectorLength(vel);
					vVelocity[1] = vBuffer[1]*projspd*GetVectorLength(vel);
					vVelocity[2] = vBuffer[2]*projspd*GetVectorLength(vel);
					TeleportEntity(entity, NULL_VECTOR, NULL_VECTOR, vVelocity);
