...
bool Weakened[MAXPLAYERS+1] = {false, ...};

new Float:Weakened_Timer[MAXPLAYERS+1] = {0.0, ...};
...
{
	if (!IsValidClient(attacker) || !IsValidClient(victim) || !IsValidEntity(ArcGun) || !IsValidClient(i) || GetClientTeam(i) == GetClientTeam(attacker))
	{
		return;
	}
	
	new Float:ChainAdditiveBonus = 0.0;
	if (!IsOnlyTarget(attacker, victim, TRay_hdl, 400.0))
	{
		...
		if (WepAttribCheck(ArcGun, "chain damage additive bonus"))
		{
			ChainAdditiveBonus = GetWepAttribValue(ArcGun, "chain damage additive bonus");
		}
		
		if (WepAttribCheck(ArcGun, "flare gun extreme"))
		{
			SDKHooks_TakeDamage(i, ArcGun, attacker, (((5.0+damage*0.79)*(1.0+Chain_Chance[attacker]))*Chain[attacker])+ChainAdditiveBonus, DMG_SHOCK, ArcGun, NULL_VECTOR, NULL_VECTOR, false);
		}
		if (WepAttribCheck(ArcGun, "arc damage chain increase per chain"))
		{
			SDKHooks_TakeDamage(i, ArcGun, attacker, (((5.0+damage*0.30)+(3*Chain[attacker]))+ChainAdditiveBonus), DMG_SHOCK, ArcGun, NULL_VECTOR, NULL_VECTOR, false);
		}
		else
		{
			SDKHooks_TakeDamage(i, ArcGun, attacker, (5.0+damage*0.79)+ChainAdditiveBonus, DMG_SHOCK, ArcGun, NULL_VECTOR, NULL_VECTOR, false);
		}
		
		if (WepAttribCheck(ArcGun, "weaken on damage chain"))
		{
			if (Weakened[i] == false)
			{
				Weakened[i] = true;
				Weakened_Timer[i] = GetEngineTime()+3.0;
			}
			if (Weakened[i])
			{
				Weakened_Timer[i] = GetEngineTime()+3.0;
...
{
	new ArcGun = GetEntPropEnt(attacker, Prop_Send, "m_hActiveWeapon");
	if (!IsValidClient(target) || TRay_hdl == INVALID_HANDLE || !IsValidEntity(ArcGun))
	{
		return false;
	}
	...
	for ( new i = 1; i <= MaxClients; i++ )
	{
		if (IsValidClient(i))
		{
			...
			if (IsValidClient(nexttarget) && GetClientTeam(nexttarget) == GetClientTeam(target))
			{
				...
				if (distance < radius){
					...
					if (IsValidTarget(nexttarget, attacker) && CanBeChained[nexttarget] == true && Chain[attacker] < ChainMax[attacker] )
					{
						if (!TR_DidHit(TRay_hdl))
						{
							...
							if (!WepAttribCheck(ArcGun, "polyute"))
							{
