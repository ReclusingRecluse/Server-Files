...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int maxlen) {
	RegPluginLibrary("custom_status_hud_Weapons");
	return APLRes_Success;
}

public void OnPluginStart() {
	g_SyncDisplay = CreateHudSynchronizer();
	g_HUDUpdateForward = CreateGlobalForward("OnCustomStatusHUDUpdate3", ET_Event, Param_Cell,
			Param_Cell);
	
	g_PrefHudXPos = RegClientCookie("statushudweapons_xpos",
			"Preferred X position for the Custom Status HUD", CookieAccess_Public);
	g_PrefHudYPos = RegClientCookie("statushudweapons_ypos",
			"Preferred Y position for the Custom Status HUD", CookieAccess_Public);
	g_PrefHudColor = RegClientCookie("statushudweapons_color",
...
void GetClientHudTextParams(int client, float &x, float &y, int &color) {
	...
	x = buffer[0]? StringToFloat(buffer) : -0.75;
