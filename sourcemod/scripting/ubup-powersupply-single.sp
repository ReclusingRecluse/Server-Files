...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
	...
	new Address:ResistanceActive = TF2Attrib_GetByName(victim, "sniper rage DISPLAY ONLY");
	new twep = GetEntPropEnt(attacker, Prop_Send, "m_hActiveWeapon");
	if (ResistanceActive!=Address_Null && fl_CurrentArmor[victim] > 0.0)
	{
		if (damagetype != 135270528)
		{
			damage /= TF2Attrib_GetValue(ResistanceActive)*0.95+(Pow(fl_CurrentArmor[victim],1.25))*0.3;
		}
		if(!strcmp("obj_sentrygun", classname))
		{
			damage /= TF2Attrib_GetValue(ResistanceActive)*0.80+(Pow(fl_CurrentArmor[victim],0.97))*0.3;
		}
	}
	if(IsValidClient(victim) && !IsFakeClient(victim))
	{
		new Address:HealonHit1 = TF2Attrib_GetByName(twep, "heal on hit for rapidfire");
		new Address:HealonHit2 = TF2Attrib_GetByName(twep, "heal on hit for slowfire");
		...
		if (HealonHit1 != Address_Null && fl_CurrentArmor[attacker] != fl_MaxArmor[attacker])
		{
			new Float:ArmorGen = TF2Attrib_GetValue(HealonHit1);
			fl_CurrentArmor[attacker] += ArmorGen;
		}
		if (HealonHit2 != Address_Null && fl_CurrentArmor[attacker] != fl_MaxArmor[attacker])
		{
			new Float:ArmorGen2 = TF2Attrib_GetValue(HealonHit2);
			fl_CurrentArmor[attacker] += ArmorGen2;
		}
	}
	if(damage < 0.0)
	{
