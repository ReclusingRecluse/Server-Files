...
#define UPDATE_URL		"https://raw.githubusercontent.com/ReclusingRecluse/Server-Files/refs/heads/main/updater%20files/uu_update.txt?token=GHSAT0AAAAAADA4KMFAWGVDD7LKDH2KXJOYZ7AYNQQ"
...
#define MAX_ATTRIBUTES 9999

#define MAX_ATTRIBUTES_ITEM 20005
...
new Handle:UbupForward;

new Handle:UbupAttribAddForward2;
...
new Handle:cvar_Botlevel;

new Handle:cvar_mvmmoneymult;

new Handle:cvar_StartMoney;

new Handle:Timers_[3];

new bool:MoneyCap_active;
...
int upgrades_removable[MAX_ATTRIBUTES];
...
public Action OnCustomStatusHUDUpdate(int client, StringMap entries) 
{
	if (IsValidClient(client))
	{
		...
		Format(money, sizeof(money), "Money: $%.0f", CurrencyOwned[client]);
		entries.SetString("a_uu_a_money", money);
	}
	return Plugin_Changed;
}

public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int maxlen) {
	RegPluginLibrary("tf2attributes_ubup");
...
public OnPluginStart()
{
	...
	UbupAttribAddForward2 = CreateGlobalForward("Ubup_OnAttribAddedWeapon", ET_Event, Param_Cell, Param_Cell, Param_Cell, Param_String);
	...
	cvar_Botlevel = CreateConVar("sm_uu_bot_scaling", "1.0", "Sets level of Bot scaling. Default: 1.0");
	cvar_mvmmoneymult = CreateConVar("sm_uu_mvm_money_mult", "1.0", "Multiplies MVM money Drops. Default: 1.0");
	RealStartMoney = GetConVarFloat(cvar_StartMoney);
	StartMoneySaved = RealStartMoney
	MoneyCap_active = false;
	for (new client = 0; client < MaxClients; client++)
	{
		if (IsValidClient(client))
		{
			...
			CurrencyOwned[client] = GetConVarFloat(cvar_StartMoney);
...
int Customclip[MAXPLAYERS+1] = 0;
int IDI[MAXPLAYERS+1] = 0;
public TF2Items_OnGiveNamedItem_Post(client, String:classname[], itemDefinitionIndex, itemLevel, itemQuality, entityIndex)
{
	...
	{
		IDI[client] = itemDefinitionIndex;
		if (itemLevel == 242)
		{
			new slot = 3;
			current_class[client] = _:TF2_GetPlayerClass(client);
		...
		else
		{
			...
			if (slot < 3)
			{
				...
				else if (current_class[client] == _:TFClass_Soldier)
				{
					if (slot == 0)
					{
						CreateTimer(0.3, Timer_Cat, slot);
						//CreateTimer(0.4, DefineAttributes, itemDefinitionIndex);
					}
					else
					{
						currentitem_catidx[client][slot] = GetUpgrade_CatList(classname);
				...
				else if (current_class[client] == _:TFClass_Sniper)
				{
					...
					//new Address:CustomClip = TF2Attrib_GetByName(client, "SET BONUS: special dsp");
					if (!strcmp(classname, "tf_weapon_sniperrifle"))
					{
						CreateTimer(0.3, Timer_Cat, slot);
					}
					else
...
//public Action:DefineAttributes
public Action:Timer_Cat(Handle:Timer, any:slot)
{
	for ( new client = 1; client <= MaxClients; client++ )
	{
		if (IsValidClient(client))
		{
			new C_Weapon = GetPlayerWeaponSlot(client,0);
			new C_Weapon1 = GetPlayerWeaponSlot(client,1);
			new C_Weapon2 = GetPlayerWeaponSlot(client,2);
			...
			if (IsValidEntity(C_Weapon))
			{
				...
				if (TF2_GetPlayerClass(client) == TFClass_Sniper)
				{
					...
					if (slot == 0)
					{
						if (WepAttribCheck(C_Weapon, "uses custom clip"))
						{
							currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_sniperrifle_customclip");
							GiveNewUpgradedWeapon_(client, slot);
						}
						else if (WepAttribCheck(C_Weapon, "tranquility"))
						{
							currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_rocketlauncher_directhit_");
							GiveNewUpgradedWeapon_(client, slot);
						}
						else
						{
				...
				{
					//GetEdictClassname(C_Weapon, classname, sizeof(classname));
					//new Address:ThreeTailedFox = TF2Attrib_GetByName(C_Weapon, "three tailed fox");
					
					if (slot == 0)
					{
						if (WepAttribCheck(C_Weapon, "three tailed fox"))
						{
							currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_rocketlauncher_threetailedfox");
							//GiveNewUpgradedWeapon_(client, slot);
						}
						else if (WepAttribCheck(C_Weapon, "void mangler"))
						{
							currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_rocketlauncher_voidmangler");
							GiveNewUpgradedWeapon_(client, slot);
						}
						else if (WepAttribCheck(C_Weapon, "eyes of tomorrow"))
						{
							currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_grenadelauncher_");
							GiveNewUpgradedWeapon_(client, slot);
						}
						else if (WepAttribCheck(C_Weapon, "solar flare"))
						{
							currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_rocketlauncher_solarflare");
							GiveNewUpgradedWeapon_(client, slot);
						}
						else
						{
				...
				{
					if (WepAttribCheck(C_Weapon, "dynamic fire rate minigun"))
					{
						currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_minigun_dynamic");
						GiveNewUpgradedWeapon_(client, slot);
					}
					else if (WepAttribCheck(C_Weapon, "xenophage"))
					{
						currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_minigun_xenophage");
						GiveNewUpgradedWeapon_(client, slot);
					}
					else if (WepAttribCheck(C_Weapon, "abbadon"))
					{
						currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_minigun_abbadon");
						GiveNewUpgradedWeapon_(client, slot);
					}
					else if (WepAttribCheck(C_Weapon, "nova mortis"))
					{
						currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_minigun_nova_mortis");
						GiveNewUpgradedWeapon_(client, slot);
					}
					else if (WepAttribCheck(C_Weapon, "thunderlord"))
					{
						currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_minigun_thunderlord");
						GiveNewUpgradedWeapon_(client, slot);
					}
					else
					{
				...
				{
					if (WepAttribCheck(C_Weapon, "last squeak"))
					{
						currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_last_squeak");
						
						//Testing Presetting attributes until I can properly fuck around wirth DefineAttributesTab function
						
						//This should set clip size penalty to 1.0, much less spammy spamm
						PresetUpgradeValue(client, slot, 4, 1.0, false);
						...
						//Set Damage penalty to 1
						PresetUpgradeValue(client, slot, 3, 1.0, false);
						
						//Set Fire rate bonus to 50%
						PresetUpgradeValue(client, slot, 1, 0.5, false);
						
						
						//Old Method
						/*
						upgrades_i_val[currentupgrades_idx[client][slot][4]] = 1.0;
						currentupgrades_val[client][slot][4] = upgrades_i_val[currentupgrades_idx[client][slot][4]];
						*/
			...
			if (IsValidEntity(C_Weapon1))
			{
				GetEdictClassname(C_Weapon1, classname, sizeof(classname));
				if (TF2_GetPlayerClass(client) == TFClass_Pyro)
				{
					if (slot == 1)
					{
						if (WepAttribCheck(C_Weapon1, "scorch shot extreme"))
						{
							currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_scorchshot");
							GiveNewUpgradedWeapon_(client, slot);
						}
						else
						{
			...
			if (IsValidEntity(C_Weapon2))
			{
				GetEdictClassname(C_Weapon2, classname, sizeof(classname));
				if (TF2_GetPlayerClass(client) == TFClass_Pyro)
				{
					if (slot == 2)
					{
						if (WepAttribCheck(C_Weapon2, "third degree extreme"))
						{
							currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_thirddegree");
							GiveNewUpgradedWeapon_(client, slot);
						}
						else if (WepAttribCheck(C_Weapon2, "forged volcanic fragment"))
						{
							currentitem_catidx[client][slot] = GetUpgrade_CatList("tf_weapon_forgedvolcanicfragment");
							GiveNewUpgradedWeapon_(client, slot)
						}
						else
						{
...
//remember, cause uu is wierd, upgrade nums are 1 less then they show in the in-game menu mkay
stock PresetUpgradeValue(client, int slot, int upgradenum, float:value, bool:ToggleRemove = false)
{
	if (!IsValidClient(client) || slot < 0){return 0;}
	
	upgrades_i_val[currentupgrades_idx[client][slot][upgradenum]] = value;
	currentupgrades_val[client][slot][upgradenum] = upgrades_i_val[currentupgrades_idx[client][slot][upgradenum]];
	
	if (!ToggleRemove){ upgrades_removable[currentupgrades_idx[client][slot][upgradenum]] = 0;}
	
	//super duper text print innit
	PrintToConsole(client, "Set upgrade %i of slot %i to %.2f", upgradenum, slot, value);
	return 1;
}
...
stock SetUpgradeName(client, int slot, int upgradenum, const char[] name)
{
	...
	new u = currentupgrades_idx[client][slot][upgradenum];
...
public CW3_OnWeaponSpawned(weapon, slot, client)
{
	...
	{
		GiveNewUpgradedWeapon_(client, slot);
		CreateTimer(0.2, WeaponReGiveUpgrades, client);
	}
}
...
//	new team = GetClientOfUserId(GetEventInt(event, "team"));
	if (!client_respawn_handled[client])
	{
		...
		if (client_respawn_checkpoint[client])
		{
			...
			CreateTimer(0.8, mvm_CheckPointAdjustCash, GetClientUserId(client));
		}
		else
		{
			CreateTimer(0.8, WeaponReGiveUpgrades, GetClientUserId(client));
		}
	}
	FakeClientCommand(client,"menuselect 0");
	CreateTimer(0.8, Timer_Cat);
	CreateTimer(0.7, BotTimer, client);
	if (IsValidClient(client))
	{
...
{
	new Float:MoneyMult = GetConVarFloat(cvar_mvmmoneymult);
	SetEventInt(event, "currency", RoundToFloor((RealStartMoney*0.005)*MoneyMult));
...
//		TF2_RespawnPlayer(client);
	...
	if(!IsMvM())
	{
		new Float:BotMoneyKill = 200.0+((SquareRoot(MoneyBonusKill + Pow(RealStartMoney, 0.9))) * 0.5) * 3.0
		new Float:PlayerMoneyKill = 200.0+((SquareRoot(MoneyBonusKill + Pow(RealStartMoney, 0.95))) * 0.7) * 3.0
		...
		if (!MoneyCap_active)
		{
			if (IsValidClient(attack, false) && IsValidClient(client) && attack != client)
			{
				...
				{ 
					if (IsClientInGame(i) && IsValidClient(i)) 
					{
						CurrencyOwned[i] += PlayerMoneyKill*1.20
						PrintToConsole(i, "+%.0f$",  PlayerMoneyKill)
					} 
				}  
				RealStartMoney += PlayerMoneyKill*1.20;
			}
			if(!IsValidClient(client) && attack != client)
			{
				...
				{ 
					if (IsClientInGame(i) && IsValidClient(i)) 
					{
						CurrencyOwned[i] += BotMoneyKill*1.20
						PrintToConsole(i, "+%.0f$", BotMoneyKill);
					} 
				}  
				RealStartMoney += BotMoneyKill*1.20
			}
		}
		/*
		if (RealStartMoney >= 900000.0 && !MoneyCap_active)
		{
			PrintToChatAll("Money Cap has been reached, kills will no longer give money. Bots will now actively scale higher in difficulty.");
			CreateTimer(120.0, BotDifficulty, _, TIMER_REPEAT);
			MoneyCap_active = true;
		}
		*/
	}
	if (IsMvM())
	{
		...
		{
			new Float:MoneyMult = GetConVarFloat(cvar_mvmmoneymult);
			new Address:IsBoss = TF2Attrib_GetByName(client, "max health additive penalty")
			new Float:BossKillBonus = 2500.0+RealStartMoney*0.008*MoneyMult;
		
			if (!IsValidClient(client) && attack != client && IsBoss != Address_Null)
			{
				for (new i = 1; i <= MaxClients; i++) 
				{ 
					if (IsClientInGame(i) && IsValidClient(i)) 
					{
						CurrencyOwned[i] += BossKillBonus
						PrintToConsole(i, "Boss Kill +%.0f$", BossKillBonus);
					} 
				}  
				RealStartMoney += BossKillBonus
			}
			else
			{
				return Plugin_Continue;
...
public Action:BotDifficulty(Handle:Timer)
{
	if(MoneyCap_active)
	{
		new Float:CurrentDiff = GetConVarFloat(cvar_Botlevel);
		
		new Float:NewDiff = CurrentDiff+0.40;
		
		SetConVarFloat(FindConVar("sm_uu_bot_scaling"), NewDiff, true, false);
	}
}
...
public Event_mvm_wave_begin(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//new Float:round = GetEntProp(ent, Prop_Send, "m_nMannVsMachineWaveCount");
	...
	for (client_id = 0; client_id < MaxClients; client_id++)
	{
		...
		//if (round % 5 == 0)
		//{
			//new Float:RoundBonus = RealStartMoney*0.35;
			//CurrencyOwned[client_id] += RoundToFloor(RoundBonus);
			//RealStartMoney += RoundToFloor(RoundBonus);
			//PrintToChat(client_id, "Round Bonus: %.0f", RoundBonus);
		//}
	}
}
...
public Action:MoneyAdjust(Handle:timer)
{
	if (Money == true)
	{
		for (new client = 0; client < MaxClients; client++)
		{
			if (IsValidClient(client))
			{
				CurrencyOwned[client] = GetConVarFloat(cvar_StartMoney);
				RealStartMoney = GetConVarFloat(cvar_StartMoney);
				CurrencySaved[client] = GetConVarFloat(cvar_StartMoney);
				Money = false;
			}
		}
	}
						...
						if (inum != 9999 && upgrades_ratio[upgrade_choice])
						{
							...
							else
							{
								...
								else
								{
									...
									GiveNewUpgradedWeapon_(mclient, arg1_)
									PrintToChat(mclient, "Successfully Brought ");
								}
							}
						}
					}
	...
	do
	{
		...
		else
		{
			...
			if (KvGetDataType(kv, NULL_STRING) != KvData_None)
			{
				...
				if (!strcmp(Buf,"removable"))
				{
					KvGetString(kv, "", Buf, 64);
					upgrades_removable[_u_id] = StringToInt(Buf);
				}
				if (!strcmp(Buf,"max"))
				{
					KvGetString(kv, "", Buf, 64);
...
public UberShopinitMenusHandlers()
{
	...
	//Timers_[2] = CreateTimer(0.1, Timer_PrintMoneyHud, _, TIMER_REPEAT);
...
public OnPluginEnd()
{
	for (new client = 1; client < MaxClients + 1; client++)
	{
		if (IsValidClient(client))
		{
			...
			for(new clientweapon = 0; clientweapon < 2; clientweapon++)
			{
				if (IsValidEntity(clientweapon) && HasEntProp(clientweapon, Prop_Send, "m_AttributeList"))
				{
					TF2Attrib_RemoveAll(clientweapon);
				}
			}
			...
			if (!IsFakeClient(client))
			{
				...
				PrintToChat(client, "UU has been reloaded, you have been moved to spectate");
			}
		}
	}
...
//	PrintToChat(client, "enter ...#resetupgradeslot %d, resetting values for %d attributes", slot, iNumAttributes);
	...
	for (i = 0; i < MAX_ATTRIBUTES; i++)
	{
		upgrades_ref_to_idx[client][slot][i] = 9999;
	}
...
//Possibly handle default weapon attributes?
...
public	Menu_TweakUpgrades_slot(mclient, arg, page)
{
	...
	&& IsPlayerAlive(mclient))
	{
		...
		for (i = 0; i < currentupgrades_number[mclient][s]; i++)
		{
			...
			else if (upgrades_removable[u] == 1 && upgrades_costs[u] > 1.0)
			{
				new nb_time_upgraded;
				nb_time_upgraded = RoundFloat(((upgrades_i_val[u] - currentupgrades_val[mclient][s][i]) / upgrades_ratio[u]));
				
				nb_time_upgraded *= -1
				new inc_ratio = RoundFloat(upgrades_costs[u] * upgrades_costs_inc_ratio[u]);
				new up_cost = ((upgrades_costs[u]+((inc_ratio) * (nb_time_upgraded - 1))/2)*nb_time_upgraded);
				up_cost /= 2;
	...
	if (IsPlayerAlive(client))
	{
		...
		*/
		...
		if (IsValidEdict(iEnt) && HasEntProp(iEnt, Prop_Send, "m_AttributeList"))
		{
			...
			if( iNumAttributes > 0 )
			{
				for( a = 0; a < iNumAttributes ; a++ )
				{
					new uuid = upgrades_to_a_id[currentupgrades_idx[client][slot][a]];
					
					//PrintToChat(client, "attribute %s", upgradesWorkNames[uuid]);
					if (strcmp(upgradesWorkNames[uuid], ""))
					{
						new String:attrib[32];
						if (iEnt == client)
						{
							TF2Attrib_SetByName(client, upgradesWorkNames[uuid], currentupgrades_val[client][slot][a]);
							Format(attrib, sizeof(attrib), "%s", upgradesWorkNames[uuid]);
							
							Action result;
							Call_StartForward(UbupForward);
						...
						else
						{
							...
							Action result;
							Call_StartForward(UbupAttribAddForward2);
							Call_PushCell(client);
							Call_PushCell(iEnt);
							Call_PushCell(activeslot);
							Call_PushString(attrib);
							Call_Finish(result);
						...
						//CreateTimer(0.1, Timer_UpgradCalc, iEnt);
					}
				}
			}
			TF2Attrib_ClearCache(iEnt);
	...
	if (cat_choice != -1)
	{
		...
		for (i = 0; i < given_upgrd_classnames_tweak_nb[w_id]; i++)
		{
			...
			for (j = 0; j < upgrades_tweaks_nb_att[tmp_spe_up_idx]; j++)
			{
				...
				{
					if (upgrades_removable[tmp_up_idx] == 1 && tmp_ratio == 0.0)
					{
						tmp_ratio = 1.0;
						plus_sign = "+";
					}
...
public MenuHandler_AttributesTweak_action(Handle:menu, MenuAction:action, client, param2)
{
	if (IsValidClient(client) && IsPlayerAlive(client) && !client_respawn_checkpoint[client])
	{
		...
		if (s >= 0 && s < 5 && param2 < MAX_ATTRIBUTES_ITEM)
		{
			if (param2 >= 0)
			{
				//PrintToChat(client,"param2 = %s from slot", param2);
				new u = currentupgrades_idx[client][s][param2]
				if (u != 20000)
				{
					if (upgrades_costs[u] < -0.0001 && upgrades_costs[u] != 0.0)
					{
						...
						if (CurrencyOwned[client] >= up_cost)
						{
							...
							PrintToChat(client,"Removed attribute %i from slot %i", param2, s);
						}
						else
						{
							new String:buffer[128]
						...
						if (up_cost == 0)
						{
							PrintToChat(client, "Attribute has already been refunded");
						}
						*/
					}
					if (upgrades_costs[u] > 1.0 && upgrades_removable[u] == 1)
					{
						new nb_time_upgraded;
						nb_time_upgraded = RoundFloat((upgrades_i_val[u] - currentupgrades_val[client][s][param2]) / upgrades_ratio[u])
					...
					else if (upgrades_i_val[u] == currentupgrades_val[client][s][param2] || upgrades_removable[u] != 1)
					{
						PrintToChat(client,"Attribute cannot be removed or has already been refunded.")
					}
...
public MenuHandler_UpgradeChoice(Handle:menu, MenuAction:action, mclient, param2)
{
	if (action == MenuAction_Select)
	{
		...
		if (is_client_got_req(mclient, upgrade_choice, slot, inum))
		{
			...
			//PrintToChat(mclient, "gave upgrade %i - %i", upgrade_choice, inum);
			GiveNewUpgradedWeapon_(mclient, slot);
		}
