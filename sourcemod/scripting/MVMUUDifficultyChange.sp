...
stock SetMVMParams()
{
	...
	if (IsValidEntity(ObjResc))
	{
		if (StrContains(responseBuffer, "Normal", true) != -1)
		{
			...
			SetConVarFloat(FindConVar("sm_uu_bot_scaling"), 0.3, true, false); //Multiply Money Given by Bots
		...
		else if (StrContains(responseBuffer, "Hard", true) != -1)
		{
			...
			SetConVarFloat(FindConVar("sm_uu_bot_scaling"), 1.6, true, false); //Multiply Money Given by Bots
		...
		else
		{
			...
			SetConVarFloat(FindConVar("sm_uu_bot_scaling"), 0.2, true, false); //Multiply Money Given by Bots
