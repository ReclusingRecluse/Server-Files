...
bool:Arc_airbornebonus_active[MAXPLAYERS+1] = {false, ...};
...
public Action:Timer_TenthSecond(Handle:Timer)
{
	for ( new client = 1; client <= MaxClients; client++ )
	{
		if (IsValidClient(client))
		{
			...
			if(WepAttribCheck(clientweapon, "airborne bonus arc"))
			{
				if (!(GetEntityFlags(client) & FL_ONGROUND))
				{
					Arc_airbornebonus_active[client] = true;
					TF2Attrib_SetByName(clientweapon, "arc damage chain increase per chain", 1.0);
					TF2Attrib_SetByName(clientweapon, "arc damage chain max", 10.0);
					TF2Attrib_SetByName(clientweapon, "throwable particle trail only", 1.0);
					TF2Attrib_SetByName(clientweapon, "polyute", 1.0);
					TF2Attrib_SetByName(clientweapon, "chain damage additive bonus", 50.0);
				}
				else
				{
					Arc_airbornebonus_active[client] = false;
					TF2Attrib_SetByName(clientweapon, "arc damage chain max", 4.0);
					TF2Attrib_SetByName(clientweapon, "throwable particle trail only", 0.45);
					TF2Attrib_RemoveByName(clientweapon, "chain damage additive bonus");
					TF2Attrib_RemoveByName(clientweapon, "polyute");
				}
			}
		}
	}
