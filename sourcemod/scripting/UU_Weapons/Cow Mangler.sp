...
Float:Totaldamage[MAXPLAYERS+1] = {1.0, ...};
...
public Action:OnTakeDamageMangler(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
	if (IsValidClient(attacker) && IsValidClient(victim))
	{
		...
		if (IsValidEntity(clientweapon))
		{
			if(WepAttribCheck(clientweapon, "void mangler"))
			{
				//damage *= (Fire_rate_to_damage[attacker]*3.0)-1.0;
		...
		if (IsValidClient(client))
		{
			...
			if (IsValidEntity(clientweapon))
			{
				if(WepAttribCheck(clientweapon, "void mangler"))
				{
					...
					Totaldamage[client] = ((Pow(Siphoned_Health[client], 0.15))*(Pow(Siphoned_Health[client], 0.07))+(Fire_rate_to_damage[client]*3.0))-2.0;
...
stock FireRateToDamage(client, c_weapon)
{
	if (IsValidEntity(c_weapon) && IsValidClient(client))
	{
		...
		{
			Fire_rate_to_damage[client] = (Pow(GetWepAttribValue(c_weapon, "fire rate bonus custom"),-0.81));
		}
		if (WepAttribCheck(c_weapon, "fire rate penalty custom"))
		{
			Fire_rate_to_damage[client] = (Pow(GetWepAttribValue(c_weapon, "fire rate penalty custom"),-0.81));
		}
		else {return;}
	}
	else {return;}
